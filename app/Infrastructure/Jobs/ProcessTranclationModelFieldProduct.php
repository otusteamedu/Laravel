...
class ProcessTranclationModelFieldProduct implements ShouldQueue
{
    use Queueable;
            ...
            foreach ($this->langs as $lang) {
                ...
                if (empty($idsModels)) {
                    $e = new NotFoundServiceException(
                        'При выполнении задачи translationModelFeildProduct не найдены записи с пустым переводом по языку ' . $lang,
                        404
                    );
                ...
                foreach ($idsModels as $idModel) {
                    try {
                        ...
                        $name = new ProductName($searchedLangValue);
