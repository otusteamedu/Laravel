...
class ProcessTranclationModelFieldMeasure implements ShouldQueue
{
    use Queueable;
    ...
    /**
     * Репозиторий для работы с моделью Measure.
            ...
            foreach ($this->langs as $lang) {
                ...
                if (empty($idsModels)) {
                    $e = new NotFoundServiceException(
                        'При выполнении задачи translationModelFeildMeasure не найдены записи с пустым переводом по языку ' . $lang,
                        404
                    );
                ...
                foreach ($idsModels as $idModel) {
                    try {
                        ...
                        $name = new MeasureName($searchedLangValue);
