...
class UserAssignTodoRoleTelegramNotofy implements ShouldQueue
{
    ...
    public function handle(UserAssignTodoRoleEvent $event): void
    {
        ...
        } catch (NotHasTelegramId) {
            //
        } catch (Exception $exception) {
            Log::error("Ошибка отправки сообщения в телеграм о назначении на роль {$event->role->value} в задаче {$event->todoId} пользователю {$event->userId}. {$exception->getMessage()}");
        }
    }
}
