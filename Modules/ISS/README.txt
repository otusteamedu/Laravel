...
ПЕРЕНОС МОДУЛЯ НА ЧИСТУЮ АРХИТЕКТУРУ
00) перенести \Tests папку в новый модуль     -- *****
        IssOrmModel2Test                      -- готов
        NotifyService2Test                    -- готов
000) перенести src\ Http в новый модуль
    Controllers
        AjaxEducationMaterialController       -- готов
        IssAdminController                    -- готов
        IssCheckExamController                -- готов
        IssRoutePointController               -- готов
        IssStartPageController                -- готов
        IssUserPageController                 -- готов
        API \v1
                EducationRouteController      -- готов
                UserDataController            -- готов
        AdminInterface                    --
                MainIssUserManageController   -- готов
                RoutePointManageController    -- готов
    Services:
        issUser -- готов
        eduMaterials -- готов
        EduRoutePoint -- готов
        EducationRoute -- готов
        Notify -- готов
        Exam -- готов
            checkSimpleExam                    -- готов
            chooseCheckType                    -- готов
            chooseExamCheckTeacher             -- готов
            delCheckCode                       -- готов
            fillExamBlank                      -- готов
            getCheckCodeByUserIdAndRealPointId -- готов
            getExamQuestions                   -- готов
            getRefPointExamQuestions           -- готов
            getUserAndPointDataByCheckCode     -- готов
            isExamCanBePassed                  -- готов
            isExamComplicated                  -- готов
            makeCheckCode                      -- готов
            markExamPassedForUser              -- готов
            processExamCheck                   -- готов
            processTeacherFeedback             -- готов


---ПЕРЕКЛЮЧЕНИЕ МОДУЛЕЙ:
    1) в bootstrap\providers.php комментим провайдер модуля и раскомментим другой провайдер модуля
    2) в bootstrap\app.php закомментить строки посредников для одного модуля и раскомментить для другого
    2) в routes\console комментим строки для модуля и раскоментим для другого модуля
    3) в vite.config закомментить пути к файлам для одного модуля и раскоментить для второго
    4) в \public\js\iss\issNodePage.js заменить адрес открытия текст и видео файлов с
       http://localhost/iss/download  на http://localhost/iss2/download
       http://localhost/iss/open      на http://localhost/iss2/open
       http://localhost/iss/checkExam на http://localhost/iss2/checkExam
       и выполнить npm run build
    5) в config\filesystem.php в в диске iss изменить путь на модуль
                               в массиве ссылок изменить путь ссылки issPublic
                               выполнить команду ./vendor/bin/sail artisan storage:link
    4) в \public\issNode.js поменять ажреса на контроллер модуля
    6) В файл phpunit.xml добавить строку
    <testsuite name="ISS2_UNIT">
        <directory>Modules/ISS/tests</directory>
    </testsuite>
