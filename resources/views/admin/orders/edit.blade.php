...
@section('content')
    ...
    <div class="row">
        <form class="mt-3 col-lg-8 col-xxl-6" action="{{ route('admin.orders.update', $orderId) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="user_id" id="orderUser">
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" @selected(old('user_id') == $user->id || $userId == $user->id)>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <div>
                    ...
                    <input type="hidden" value="{{ $orderProduct->getId() }}" name="product_id[]">
                    <div class="input-group mt-2">
                        <span class="input-group-text">Товар и количество</span>
                        <input type="text" class="form-control" disabled value="{{ $orderProduct->getTitle() }}"">
                        <input type="number" min="0" max="100" name="count[]" class="form-control" value="{{ $orderProduct->pivot->count }}" required>
                    </div>
                    @endforeach
                </div>
            </div>
