...
<div class="mb-4">
    ...
    <input type="number" id="order" name="order" value="{{ old('order', $category->order ?? '0') }}"
...
<div class="mb-4">
    ...
    <textarea id="text" name="text" rows="5"
              class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('text') border-red-500 @enderror">{{ old('text', $category->text ?? '') }}</textarea>
    @error('text')
    <p class="text-red-500 text-xs italic">{{ $message }}</p>
    @enderror
</div>
...
<div class="mb-4">
    ...
    <label class="inline-flex items-center">
        <input type="checkbox" name="published" value="1" class="form-checkbox h-5 w-5 text-blue-600"
            {{ old('published', $category->published ?? false) ? 'checked' : '' }}>
