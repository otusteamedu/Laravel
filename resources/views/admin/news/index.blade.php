...
@php
    /**
     * @var App\Services\News\Results\Result[] $news
     */
@endphp
{{--@dd($news)--}}
@section('content')
                        ...
                        <tbody>
                            @foreach($news as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>
                                        <a href="{{ route('admin.news.edit', $item->id) }}" class="text-decoration-none fw-bold">
                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm">
                                            <a href="{{ route('admin.news.show', $item->id) }}" class="btn btn-outline-primary" target="_blank" title="Просмотр">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('admin.news.edit', $item->id) }}" class="btn btn-outline-secondary" title="Редактировать">
                                                <i class="fas fa-edit"></i>
                                            </a>
                                            <form action="{{ route('admin.news.destroy', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Вы уверены? Это действие нельзя отменить.')">
                                                @csrf
                                                @method('DELETE')
